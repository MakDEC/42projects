...
/*   Updated: 2018/08/16 16:28:07 by aanger           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (filestat[0].st_ctime > filestat[1].st_ctime ||
	(filestat[0].st_ctime == filestat[1].st_ctime
...
{
	if (filestat[0].st_ctime < filestat[1].st_ctime && (debut = new) > 0)
		new->next = dep;
	else
		swit(new, dep);
	return (debut);
...
t_infos				*arg_c(t_infos *new, t_infos *debut, char *dir)
{
	...
	while (dep->next != NULL && filestat[0].st_ctime > filestat[1].st_ctime)
	{
		swap = dep;
		dep = dep->next;
		prep_filestat(dir, dep, &filestat[0]);
