...
/*   Updated: 2018/08/16 16:21:47 by aanger           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (filestat[0].st_atime > filestat[1].st_atime ||
	(filestat[0].st_atime == filestat[1].st_atime
...
{
	if (filestat[0].st_atime < filestat[1].st_atime && (debut = new) > 0)
		new->next = dep;
	else
		swit(new, dep);
	return (debut);
...
t_infos				*arg_u(t_infos *new, t_infos *debut, char *dir)
{
	...
	while (dep->next != NULL && filestat[0].st_atime > filestat[1].st_atime)
	{
		swap = dep;
		dep = dep->next;
		prep_filestat(dir, dep, &filestat[0]);
